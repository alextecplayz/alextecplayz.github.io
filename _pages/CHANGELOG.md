...
## [2026.02] | UI improvements, no JS

- The website does not use client-side JavaScript anymore. Yes, I had to remove Fedi comment support and the Search function (which I'll admit was a nice idea at the time, *especially with my attempt at a full-text search that offered snippets of the page!*) but I kept adding more features via JS instead of trying to use HTML and CSS where possible. And because I'm using a static-site generator, I don't need to use client-side JS anymore. This means that ALL of my site is completely usable without JavaScript, unlike a *concerning* amount of blogs and sites I visit, whose JS gets implicitly blocked by the NoScript extension and uBlock Origin via strict mode.

{% aside %}
**If I can't even open your hamburger menu or open your Table of Contents without JS, you know your site is fucked and you need to return to HTML roots, \<details> exists for a fucking reason, use it!**
{% endaside %}

- Because no JS is used, things like themes and effects persist only for the current page. Rain and snow have also been removed for now, since I now use strict CSP headers via Cloudflare rule transforms, and inline CSS is not recommended. *Userscript managers will throw errors when you visit my site, same for Stylus and other CSS or JS inliners. Add exceptions to my site or live with the errors.*
- /now was cut, since I can't for the life of me update it in a timely manner
- /recommends is now [/collections/websites.html]({{site.baseurl}}/collections/websites.html)

## [2026] | "Starlight" - Hello, 11ty (January 2026)
- Moved the site from Jekyll to 11ty. Quite easy to transition(🏳️‍⚧️?) from Ruby/Jekyll to 11ty, but I had to rewrite my plugins from Ruby to JS.
...
- nonce-gen.rb Jekyll plugin written to generate the nonce every time the site is built, this value is used by the `nonce` Liquid value
